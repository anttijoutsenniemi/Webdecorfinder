...
const ChatApp: React.FC<ChildComponentProps> = ({
  appStates,
  navigateHandler,
  phaseNumber,
  setModalOpen,
  ...
  const updateImage = (img64: string) => {
    ...
    setTimeout(() => {
      //timeout to let rendering happen first before autoscroll
      scrollToBottom();
    }, 50);
  };
  ...
  const initiateWebSearch = async () => {
    try {
      ...
      const queryObject = await fetchInterPretationForWebSearch(
        refImageArray,
        appStates.furnitureClass
      );
      const parsedQueryObject =
        typeof queryObject === "string" ? JSON.parse(queryObject) : queryObject;

      const newQuery = `${parsedQueryObject.webSearchQuery} `;
      ...
      if (recommendations && recommendations.length > 0) {
        ...
        handleOptionClick(
          "suositukset",
          "Voisitko näyttää minulle kalustesuositukset?",
          top12matches,
          botAnswr
        );
      } else {
  ...
  const handleOptionClick = (
    option: string,
    userMessage?: string,
    recommendations?: CompareObject[],
    botAnswr?: string
  ) => {
    const newUserMessage: ChatMessage = {
      id: appStates.messages.length + 1,
      type: "user",
      text: userMessage ? userMessage : option,
    }; // ternäärinen ehto lähettää käyttäjän viestin kuplana, kun käyttäjä kirjoittaa ja lähettää
    ...
    switch (option) {
      case "1. Etsi kalusteita verkosta":
        ...
        setTimeout(() => {
          //this wont work without timeout for some reason
          handleOptionClick(
            "1. Etsi kalusteita käyttämällä kuvia tilasta",
            "Etsi kalusteita verkosta"
          );
        }, 50);
        break;
      case "1. Etsi kalusteita käyttämällä kuvia tilasta":
        botResponseText =
          "Hienoa, aloitetaan! Mikä on lähin kaupunkisi? Tämä auttaa minua etsimään kalusteita läheltä sinua.";
        setTypingPhase(1);
        setTypingMode(true);
        nextPageNumber = phaseNumber + 1;
        break;
      ...
      case "Ei tuotteita":
        setLoading(false);
        botResponseText =
          "Näyttää siltä, ettei valitsemastasi kategoriasta löytynyt tällä hetkellä tarpeeksi käytettyjä tuotteita. Ne saattavat olla loppuunmyytyjä, ja saatat löytää niitä kokeilemalla myöhemmin uudestaan. Haluaisitko etsiä saman kategorian kalusteita uusista tuotteista?";
      ...
      case "suositukset":
        if (botAnswr && recommendations) {
          botResponseText =
            botAnswr +
            " Löysin nämä alla olevat suositukset, jotka sopivat mielestäni parhaiten tyyliisi. Jos suositukset eivät tällä kertaa osuneet kohdalleen, voimme myös halutessasi etsiä uusista tuotteista tai eri kategoriasta!";
      ...
      default:
        ...
        if (categories.includes(option)) {
          ...
          if (appStates.webSearchMode) {
            ...
            options = [
              "Lisää kuva/kuvia tilasta",
              "Etsitään satunnaisia suosituksia",
            ];
          }
          nextPageNumber = phaseNumber + 1;
        }
        ...
        <div className="chat-wrapper">
            ...
            <div key={message.id} className={`chat-message ${message.type}`}>
              {message.type === "chatbot" && (
                <div className="chat-content">
                  ...
                  <div>
                    <div className="chat-bubble" ref={appStates.messageEnd}>
                      {message.text}
                    </div>

                    {
                      ...
                      message.recommendationArray &&
                        message.recommendationArray.length > 0 && (
                          <>
                            <ProductCard
                              products={message.recommendationArray}
                              onCardClick={openProductInStore}
                            />
                            <Modal
                              title="Select from options below"
                              product={appStates.selectedProduct}
                              isOpen={appStates.modalOpen}
                              onClose={closeModal}
                            />
                          </>
                        )
                    ...
                    {message.options &&
                    message.id === appStates.messages.length ? ( //only render options on the last message so user cant click previous options
                      <>
          ...
          {appStates.loading && (
            <div className="loadingWrapper">
              ...
              <p>
                Hetkinen... analysoin antamiasi tietoja ja etsin sopivia
                kalusteita
              </p>
            </div>
          )}
          {appStates.errorMessage && (
            <div>
              <p style={{ color: "red" }}>{appStates.errorMessage}</p>
            </div>
          )}
          {appStates.typingMode && (
            <InputField
              receiveInput={receiveInput}
              typingPhase={appStates.typingPhase}
            />
          )}
          {currentPhase > 0 && (
